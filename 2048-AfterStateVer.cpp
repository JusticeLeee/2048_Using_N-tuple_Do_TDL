...
class board {
public:
	board(uint64_t raw = 0) : raw(raw) {} // default constructor
	board(const board& b) = default; // copy constructor
	board& operator =(const board& b) = default; // assignment operator
...
public:
	...
	int move_left() {
		...
		raw = move;
		return (move != prev) ? score : -1; // 可以動 就return score 否則 return -1
	}
	int move_right() {
		uint64_t move = 0;
		uint64_t prev = raw;
	...
	void reverse() { mirror(); flip(); } //整個顛倒 0x0123456789abcdef -> 0xfedcba9876543210
    ...
    friend std::ostream& operator <<(std::ostream& out, const board& b) {
		...
		for (int i = 0; i < 16; i += 4) {
			snprintf(buff, sizeof(buff), "|%6u%6u%6u%6u|",
				(1 << b.at(i + 0)) & 0xfffe, // use -2u (0xff...fe) to remove the unnecessary 1 for (1 << 0)
				(1 << b.at(i + 1)) & 0xfffe,
				(1 << b.at(i + 2)) & 0xfffe,
				(1 << b.at(i + 3)) & 0xfffe);
			out << buff << std::endl;
...
public:
	...
	virtual float update(const board& b, float u) {
		// TODO
		float value=0;
		float u_split = u / isomorphic.size();
		for(int i=0 ; i<8 ; i++){
			size_t index = indexof(isomorphic[i],b);
			v[index] += u_split;
			value += v[index];
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		...
		size_t index=0;
		for (int i=0;i<6;i++){
			index = index | (b.at(patt[i])<< i * 4 );//it should be like 0x(1111) -> 0x(1111)(2222)
		}
		return index;
	}
...
int main(int argc, const char* argv[]) {
	...
	// train the model
	std::vector<state> path; //一個裝state的vector
	path.reserve(20000); // 指定vector 長度 20000
